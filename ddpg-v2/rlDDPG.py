...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound, outputGraph=True, saveModel=True):
        self.memory = np.zeros(
            (MEMORY_CAPACITY, s_dim * 2 + a_dim * 2), dtype=np.float32)
        self.pointer = 0
        self.best_r = 0
    ...
    def _build_a(self, s, scope, trainable):
        ...
        with tf.variable_scope(scope):
            layer1 = tf.layers.dense(s, hidden1, activation=None, kernel_initializer=init_w,
                                     bias_initializer=init_b, name='l1', trainable=trainable)
            layer1 = tf.layers.batch_normalization(
                layer1, training=trainable, name="norms1")
            layer2 = tf.layers.dense(layer1, hidden2, activation=None, kernel_initializer=init_w,
                                     bias_initializer=init_b, name='l2', trainable=trainable)
            layer2 = tf.layers.batch_normalization(
                layer2, training=trainable, name="norms2")
            a = tf.layers.dense(layer2, self.a_dim, activation=tf.nn.tanh, name='a',
    ...
    def _build_c(self, s, a, scope, trainable):
        ...
        with tf.variable_scope(scope):
            hidden1 = 256
            hidden2 = 128
            layer_s = tf.layers.dense(s, hidden1, activation=None, kernel_initializer=init_w,
                                      bias_initializer=init_b, name='l1_s', trainable=trainable)
            layer_s = tf.layers.batch_normalization(
                layer_s, training=trainable, name="norms1")
            layer_s = tf.layers.dense(layer_s, hidden2, activation=None, kernel_initializer=init_w,
                                      bias_initializer=init_b, name='l2_s', trainable=trainable)
            layer_s = tf.layers.batch_normalization(
                layer_s, training=trainable, name="norms2")
            layer_a = tf.layers.dense(
                a, hidden2, activation=None, name='a', trainable=trainable)
            net = tf.layers.dense(tf.keras.layers.concatenate(
                [layer_s, layer_a]), hidden2, activation=tf.nn.relu, name='cat', kernel_initializer=init_w, bias_initializer=init_b, trainable=trainable)
            net = tf.layers.dense(net, self.a_dim, activation=None, name='Q',
                                  kernel_initializer=init_w, bias_initializer=init_b, trainable=trainable)
            return net

    def save_Model(self, DIR, reward, episode):
        print(
            "best_reward:{:+.3f}, reward:{:+.3f}".format(self.best_r, reward))
...
def step(action, lineNum):
    ...
    vrep.simxSetIntegerSignal(
        clientID, "line", lineNum, vrep.simx_opmode_blocking)
    ...
    error, next_angle = vrep.simxGetFloatSignal(
        clientID, "angle", vrep.simx_opmode_buffer)
    error, next_velocity = vrep.simxGetFloatSignal(
        clientID, "velocity", vrep.simx_opmode_buffer)
    error, reward = vrep.simxGetFloatSignal(
        clientID, "reward", vrep.simx_opmode_buffer)
    error, done = vrep.simxGetIntegerSignal(
        clientID, "done", vrep.simx_opmode_buffer)
    # integrate the angle and the velocity to the next obs
    observation_ = np.array([next_angle, next_velocity], dtype=np.float32)
    print("lineNum{}|  angle:{:+.3f}  velocity:{:+.3f}  reward:{:+.3f}  done:{}".format(
        str(lineNum).zfill(3), next_angle*180/math.pi, next_velocity*180/math.pi, reward, done))
...
def reset():
    ...
    error, angle = vrep.simxGetFloatSignal(
        clientID, "angle", vrep.simx_opmode_buffer)
    error, velocity = vrep.simxGetFloatSignal(
        clientID, "velocity", vrep.simx_opmode_buffer)
...
def stream():
    ...
    error, next_angle = vrep.simxGetFloatSignal(
        clientID, "angle", vrep.simx_opmode_streaming)
    error, next_velocity = vrep.simxGetFloatSignal(
        clientID, "velocity", vrep.simx_opmode_streaming)
    error, reward = vrep.simxGetFloatSignal(
        clientID, "reward", vrep.simx_opmode_streaming)
    error, done = vrep.simxGetIntegerSignal(
        clientID, "done", vrep.simx_opmode_streaming)
        ...
        for episode in range(MAX_EPISODES):
            while True:
                ...
                vrep.simxGetIntegerSignal(
                    clientID, 'line', vrep.simx_opmode_blocking)
